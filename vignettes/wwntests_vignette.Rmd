...
We draw up some simple expositionary examples with a sample of brownian motion curves and a sample of FAR(1,0.75)-IID curves (which are conditionally heteroscedastic).
```{r}
library(wwntests)
set.seed(1234)
...
We try the single-lag tests with a lag of 1, and the multi-lag test with a maximum lag of 10 (note, the default signficance level is $\alpha = 0.05$) on our functional Brownian motion and FAR data using the *fport_test* function and passing the string handles 'single-lag' and 'multi-lag' to the *test* parameter. For the single-lag test, the *lag* parameter determines the lag of the of the autocovariance function, and for the multi-lag test, it determines the maximum lag to include in $V_{T,K}$ (that is, lag = $K$).
...
The single-lag test, and in particular, the multi-lag test, are computationally expensive. Another supported test, referred to by its string handle 'spectral', which is significantly faster. The drawback of this test, is thata it is not built for general white noise (e.g. functional conditionally heterscedastic) series. It is based on the spectral density operator $\mathscr{F}(\omega) = \frac{1}{2\pi} \sum_{j \in \mathbb{Z}} C(j)e^{-ij\omega}, \omega \in [-\pi, \pi]$, where $C(j)$ are the autocovariance operators, $C(j) = E[X_j \otimes X_0], j \in \mathbb{Z}$. These operators are esimated by $\hat{C}_n(j) = \frac{1}{n} \sum_{t = j+1}^n u_t \otimes u_{t-j}, 0 \le j < n$ and $\hat{\mathscr{F}}_n(\omega) = \frac{1}{2\pi} \sum_{|j|<n} k(\frac{j}{p_n})\hat{C}_n(j)e^{-ij\omega}, \omega \in [-\pi, \pi]$, where $k$ is a user-chosen kernel function and $p_n$ is the bandwidth parameter (or lag-window); it may either me a user-inputter positive integer, computed from the sample size via $p_n = n^{\frac{1}{2q+1}}$, or computed via a data-adaptive process (please see Kokoszka, Rice, Shang for a complete treatment of this process).
Currently supported kernel functions are the Barlett, Parzen, and Daniell kernels:
$$
\begin{align*}
k_B(x) &= \begin{cases}
1 - |x| & \text{ for } |x| \le 1 \\
0 & \text{ otherwise }
\end{cases} & \text{(Barlett)}
\\
k_P(x) &= \begin{cases}
...
k_D(x) &= \frac{\sin(\pi x)}{\pi x}, x \in \mathbb{R} \setminus \{0\}, k_D(0) = 1 & \text{(Daniell)}
\end{align*}
$$
...
$$
, where $\hat{\sigma}^2_n = n^{-1} \sum_{t=1}^n ||X_t||^2$, $C_n(k) = \sum_{j=1}^{n-1}(1 - \frac{j}{n})k^2(\frac{j}{p_n})$, and $D_n(k) = \sum_{j=1}^{n-2} (1 - \frac{j}{n})(1 - \frac{j+1}{n})k^4(\frac{j}{p_n})$. We actually use a power transformation of this test statistic proposed by Chen and Deo ***FOOTNOTE***, but this is quite involved and we direct the reader again to Kokoszka, Rice and Shang.
...
```
```{r}
fport_test(b, test='spectral', kernel = 'Daniell', bandwidth = 3, suppress_raw_output = TRUE)
```
```{r}
...
Performs a test for independence and identical distribution of functional observations. The test relies on a dimensional reduction via a projection of the data on the K most important functional pricnipal components. The empirical autocovariance operator is given by
...
The 'components' parameter (denoted by p above) determines how many functional principal componenents to use (kept in order of importance, which is determined by the proportion of the variance that each computed component explains). The 'lag' parameter (denoted by H above) determines the maximum lag to consider.
...
Passing *suppress_print_output* = TRUE will cause the function to omit any output to the console. Passing *suppress_raw_output* = TRUE will cause the function to not return the list. Atleast one of these parameters must be TRUE.
